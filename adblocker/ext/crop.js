(function($) {
    ...
    function cropImage(save) {
        ...
        if (save) {
        console.log(format, quality/100);
    ...
    function createCoords(c) {
        ...
        var btns = $('<div/>', {
            'id': 'screenshotbutton',
            'class': 'stopallads_screenshot_buttons'
        });

        btns.append(btne);
        btns.append('<a class="edit_btn save" id="imgdownload" href="" download="screenshot-by-stopallads"><i class="save"></i><div class="name">'+chrome.i18n.getMessage("cropBtnSave")+'</div></a>');
        btns.append(btnc);
...
//        });
        ...
        var btnstopallads = $('<button/>', {
            html: '<i class="stopallads"></i>',
            'id': 'screenshotstopallads',
            'class': 'edit_btn stopallads',
            'title': chrome.i18n.getMessage("cropBtnstopallads")
        });
//        btnsdone.append(btncopy);
        btnsdone.append(btnstopallads);
...
//        });
        ...
        btnstopallads.bind('click', function() {
            chrome.extension.sendMessage({msg: 'sendtostopallads', img: $('#imgdownload').attr('href')}, function(response) {
                console.log(response);
            });
            destroyCrop();
        });
    ...
    if (!window.screencaptureinit) {
        ...
        chrome.extension.onRequest.addListener(function(req) {
            ...
            if (req.msg == 'crop') {
                ...
                chrome.extension.sendMessage({msg: 'getformat'}, function(response) {alert('1');
                ...
                chrome.extension.sendMessage({msg: 'getCropPosition'}, function(response) {
                    if (typeof response === 'object') {alert('2');
