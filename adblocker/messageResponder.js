  ...
  global.ext.onMessage.addListener(function(message, sender, callback)
  {
    switch (message.type)
    {
      ...
      case "subscriptions.toggle":
        ...
        {
            if(subscription.url == "http://cdn.systweak.com/stopallads/subscriptions/AntiAdBlockerListSAA.txt")
            {
                //Main version of StopAll Ads Killer script
                //Remove Lite before Main load
                var LiteSubscription = Subscription.fromURL("http://cdn.systweak.com/stopallads/subscriptions/StopAllAdsKillerScripts.txt");    
                FilterStorage.removeSubscription(LiteSubscription);
            }          
          subscription.disabled = false;
          subscription.title = message.title;
          subscription.homepage = message.homepage;
