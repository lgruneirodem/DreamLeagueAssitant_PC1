...
{
  // Set page title to i18n version of "StopAll Ads Options"
  ...
  fetch("subscriptions.xml")
    ...
    .then(function(text)
    {
      ...
      for (var i = 0; i < elements.length; i++)
      {
        ...
        if(option._data.title == "StopAll Ads - Filtered Malwares" || option._data.title == "Fanboy's Social Blocking List" || option._data.title == "EasyList" || option._data.title == "Anti-Adblock Killer (Lite)" || option._data.title == "EasyList+Malware")
        {
          option.style.fontWeight = "bold";
        }
...
function addSubscriptionClicked()
{
  ...
  var data = list.options[list.selectedIndex]._data;
  if (data){
    if(data.url == "http://cdn.systweak.com/stopallads/subscriptions/AntiAdBlockerListSAA.txt")
    {
      //Main version of StopAll Ads Killer script
      //Remove Lite before Main load
      removeSubscription("http://cdn.systweak.com/stopallads/subscriptions/StopAllAdsKillerScripts.txt");
    }
    else if(data.url == "http://cdn.systweak.com/stopallads/subscriptions/StopAllAdsKillerScripts.txt")
    {
      //Lite version of StopAll Ads Killer script 
      //Remove Main before Lite load
      removeSubscription("http://cdn.systweak.com/stopallads/subscriptions/AntiAdBlockerListSAA.txt");
    }
